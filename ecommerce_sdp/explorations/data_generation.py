...
for i in range(1, 5001):
    order_data.append({
        ...
        "order_date": (datetime(2025, 1, 1) + timedelta(days=random.randint(0, 365))).strftime("%Y-%m-%d"),
...
for i in range(1, 201):
    ...
    cdc_events.append({
        "cdc_timestamp": (datetime(2025, 6, 1) + timedelta(hours=random.randint(0, 4000))).strftime("%Y-%m-%d %H:%M:%S"),
        "operation": op,
        "customer_id": cust_id,
